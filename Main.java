...
public class Main {
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            if (player[l].isInPrison()) {
                System.out.println(player[l].getPlayerNick() + "You are in prison. You skip turn unless you have pass.");
    ...
    private static void chanceOrCommunityChest() {
        if (board.isChancePos(pos)) {
            player[l].drawCard();
        }
    }

    private static void normalPlace() {
        if (board.isFree(pos)) {
            board.isBuying(pos);
            if (board.getIsBuying()) {
                if (player[l].hasEnoughMoney(board.placePrize(pos))) {
    ...
    private static boolean jailBlock() {
        if (board.getsInJail(pos)) {
            System.out.println("You are now in prison for your bad crimes.");
            player[l].getInPrison();
            nextPlayer(l, playerNumber);
